...
export default function PostPage() {
    ...
    const getpost = async () => {
        try {
            const response = await axios.get(`http://localhost:5000/api/post/get/PostID/${postid}`,
                {
                    headers: {
                        jwttoken: jwt_here
                    }
            ...
            const response2 = await axios.get(`http://localhost:5000/api/user/post/user/details/${response.data.user}`,
                {
                    headers: {
                        jwttoken: jwt_here
                    }
